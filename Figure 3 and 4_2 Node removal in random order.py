...
output_dir = r"E:\论文写作\35 TRE\代码\1 去点_单次\2 去点_单次_随机\数据"
figure_dir = r"E:\论文写作\35 TRE\代码\1 去点_单次\2 去点_单次_随机\作图"
...
# 生成随机去点顺序
random_removal_order_nodes = nodes_list.copy()
np.random.shuffle(random_removal_order_nodes)

# 保存随机去点顺序
node_removal_order_df = pd.DataFrame({
    'Removal_Order': range(1, len(random_removal_order_nodes) + 1),
    'Node': random_removal_order_nodes,
    'Weight': [node_trade_volumes[node] for node in random_removal_order_nodes]
})
removal_order_file = os.path.join(output_dir, "random_node_removal_order.csv")
node_removal_order_df.to_csv(removal_order_file, index=False)
print(f"随机去点顺序已保存至: {removal_order_file}")
...
def theoretical_robustness_analysis_node_removal(G_lcc, nodes, node_in_volumes, node_out_volumes, B, x_mean, removal_order_nodes):
    """改进的理论解析韧性分析 - 使用进口和出口分离的动力学，分母改为x_mean，随机去点"""
...
# 7. 改进的韧性分析 - 基于动力学方程的仿真方法（分开进口和出口，使用B_i，分母改为x_mean，随机去点）
def simulate_dynamics_robustness_node_removal(G_lcc, nodes, node_in_volumes, node_out_volumes, E, G_matrix, B_i_values, x_mean, removal_order_nodes):
    """基于改进动力学方程的仿真韧性分析 - 分开进口和出口，使用B_i，分母改为x_mean，随机去点"""
    n = len(nodes)
...
theoretical_q_values, theoretical_results_total, steady_state_solutions_total, steady_state_solutions_in, steady_state_solutions_out, alpha_values, beta_values = theoretical_robustness_analysis_node_removal(
    G_lcc, nodes, node_in_volumes, node_out_volumes, B_solution, x_mean, random_removal_order_nodes)

print("开始基于改进动力学方程的仿真韧性分析（随机去点）...")
simulation_q_values, simulation_avg_total_steady, simulation_avg_in_steady, simulation_avg_out_steady, all_country_steady_states_total, all_country_steady_states_in, all_country_steady_states_out, all_country_trajectories_total, step1_trajectories, step10_trajectories, simulation_alpha_values, simulation_beta_values = simulate_dynamics_robustness_node_removal(
    G_lcc, nodes, node_in_volumes, node_out_volumes, E, G_matrix, B_i_values, x_mean, random_removal_order_nodes)
...
ax2.legend(fontsize=font_size-8, loc='center right', bbox_to_anchor=(0.65, 0.4))
...
if step1_trajectories is not None:
    ...
    plt.xticks(fontsize=font_size-4)
    plt.yticks(fontsize=font_size-4)
    plt.legend(fontsize=font_size-4, loc='upper right', bbox_to_anchor=(0.98, 0.98))
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
# 15. 输出改进的韧性动力学方程
print("\n=== 改进的韧性动力学方程（进口和出口分离，使用B_i，分母改为x_mean，随机去点）===")
print("原始方程:")
