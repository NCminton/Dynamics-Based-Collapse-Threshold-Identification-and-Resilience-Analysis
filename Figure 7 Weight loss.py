...
output_dir = r"E:\论文写作\35 TRE\代码\3 权重丢失\数据"
figure_dir = r"E:\论文写作\35 TRE\代码\3 权重丢失\作图"
...
# ================ 修改部分：改为权重丢失方式 ================
# 定义权重丢失比例ql的范围（从1到0）
ql_steps = 51  # 包括ql=1和ql=0
ql_values = np.linspace(1, 0, ql_steps)

# 计算原始总边权重
edges = list(G_lcc.edges(data=True))
total_edge_weight = sum([edge[2]['weight'] for edge in edges])
print(f"总边权重: {total_edge_weight:.4f}")

# 保存权重丢失的顺序
weight_loss_order_df = pd.DataFrame({
    'Step': range(1, ql_steps + 1),
    'ql': ql_values,
    'Weight_Remaining': ql_values,
    'Weight_Lost': 1 - ql_values
})
loss_order_file = os.path.join(output_dir, "weight_loss_order.csv")
weight_loss_order_df.to_csv(loss_order_file, index=False)
print(f"权重丢失顺序已保存至: {loss_order_file}")

# 5. 改进的韧性分析 - 基于理论解析的方法（使用平均B）- 修改为权重丢失
def theoretical_robustness_analysis_weight_loss(G_lcc, nodes, node_in_volumes, node_out_volumes, B, x_mean, ql_values):
    """改进的理论解析韧性分析 - 使用进口和出口分离的动力学，分母改为x_mean，权重丢失"""
    ...
    alpha_values = []  # 存储每个ql值对应的alpha
    beta_values = []  # 存储每个ql值对应的beta
    
    print("开始理论解析韧性分析（权重丢失）...")
    
    for ql in tqdm(ql_values, desc="权重丢失进度"):
        # 计算当前权重丢失后的平均进口量和平均出口量
        # 权重丢失后，每个节点的进出口量都会按比例减少
        alpha_current = ql * x_in_mean
        beta_current = ql * x_out_mean
    ...
    # 输出关键点的结果
    print(f"\n关键点结果:")
    print(f"ql=1.0 (原始网络): alpha={alpha_values[0]:.4f}, beta={beta_values[0]:.4f}, 总稳态={theoretical_results[0]:.4f}")
    print(f"ql=0.5 (权重丢失50%): alpha={alpha_values[ql_steps//2]:.4f}, beta={beta_values[ql_steps//2]:.4f}, 总稳态={theoretical_results[ql_steps//2]:.4f}")
    print(f"ql=0.0 (所有权重丢失): alpha={alpha_values[-1]:.4f}, beta={beta_values[-1]:.4f}, 总稳态={theoretical_results[-1]:.4f}")
    
    return ql_values, theoretical_results, steady_state_solutions, steady_state_solutions_in, steady_state_solutions_out, alpha_values, beta_values
...
# 7. 改进的韧性分析 - 基于动力学方程的仿真方法（分开进口和出口，使用B_i，分母改为x_mean，权重丢失）
def simulate_dynamics_robustness_weight_loss(G_lcc, nodes, node_in_volumes, node_out_volumes, E, G_matrix, B_i_values, x_mean, ql_values):
    """基于改进动力学方程的仿真韧性分析 - 分开进口和出口，使用B_i，分母改为x_mean，权重丢失"""
    n = len(nodes)
    
    # 创建节点到索引的映射
    node_to_idx = {node: i for i, node in enumerate(nodes)}
    
    # 记录每个ql值对应的所有国家演化曲线（进口）
    all_country_steady_states_in = []
    all_country_trajectories_in = []
    
    # 记录每个ql值对应的所有国家演化曲线（出口）
    all_country_steady_states_out = []
    all_country_trajectories_out = []
    
    # 记录每个ql值对应的所有国家总贸易量演化曲线
    all_country_steady_states = []
    all_country_trajectories = []
    
    # 记录每个ql值对应的网络平均进口量和平均出口量（从网络结构计算）
    ...
    # 记录关键点的演化曲线
    ql1_trajectories = None  # ql=1.0（原始网络）
    ql05_trajectories = None  # ql=0.5（权重丢失50%）
    ql01_trajectories = None  # ql=0.1（权重丢失90%）
    ...
    for idx, ql in enumerate(tqdm(ql_values, desc="权重丢失仿真进度")):
        # 应用权重丢失：将原始矩阵乘以ql
        E_current = E * ql
        G_current = G_matrix * ql
        ...
        # 记录关键点的演化曲线
        if idx == 0:  # ql=1.0
            ql1_trajectories = country_trajectories
        elif idx == ql_steps // 2:  # ql=0.5
            ql05_trajectories = country_trajectories
        elif ql == 0.1:  # ql=0.1
            ql01_trajectories = country_trajectories
    
    print(f"\n仿真完成！")
    print(f"ql=1.0: 仿真总稳态={simulation_avg_total_steady[0]:.4f}")
    print(f"ql=0.5: 仿真总稳态={simulation_avg_total_steady[ql_steps//2]:.4f}")
    print(f"ql=0.0: 仿真总稳态={simulation_avg_total_steady[-1]:.4f}")
    
    return (ql_values, simulation_avg_total_steady, simulation_avg_in_steady, simulation_avg_out_steady,
            all_country_steady_states, all_country_steady_states_in, all_country_steady_states_out,
            all_country_trajectories, ql1_trajectories, ql05_trajectories, ql01_trajectories,
            simulation_alpha_values, simulation_beta_values)

# 8. 执行韧性分析
print("开始改进的理论解析韧性分析（权重丢失）...")
theoretical_ql_values, theoretical_results_total, steady_state_solutions_total, steady_state_solutions_in, steady_state_solutions_out, alpha_values, beta_values = theoretical_robustness_analysis_weight_loss(
    G_lcc, nodes, node_in_volumes, node_out_volumes, B_solution, x_mean, ql_values)

print("\n开始基于改进动力学方程的仿真韧性分析（权重丢失）...")
simulation_ql_values, simulation_avg_total_steady, simulation_avg_in_steady, simulation_avg_out_steady, all_country_steady_states_total, all_country_steady_states_in, all_country_steady_states_out, all_country_trajectories_total, ql1_trajectories, ql05_trajectories, ql01_trajectories, simulation_alpha_values, simulation_beta_values = simulate_dynamics_robustness_weight_loss(
    G_lcc, nodes, node_in_volumes, node_out_volumes, E, G_matrix, B_i_values, x_mean, ql_values)

# 9. 保存结果
# 保存韧性分析结果
results_df = pd.DataFrame({
    'Weight_Loss_Ratio': 1 - simulation_ql_values,  # ql: 剩余权重比例，1-ql: 丢失权重比例
    'Weight_Remaining_Ratio': simulation_ql_values,  # ql: 剩余权重比例
    'Theoretical_Resilience_Total': np.interp(simulation_ql_values, theoretical_ql_values, theoretical_results_total),
    'Theoretical_Resilience_In': np.interp(simulation_ql_values, theoretical_ql_values, steady_state_solutions_in),
    'Theoretical_Resilience_Out': np.interp(simulation_ql_values, theoretical_ql_values, steady_state_solutions_out),
...
results_file = os.path.join(output_dir, "resilience_analysis_results_weight_loss.csv")
results_df.to_csv(results_file, index=False)
print(f"韧性分析结果已保存至: {results_file}")

# 保存每个ql值对应的解
solutions_df = pd.DataFrame({
    'Weight_Loss_Ratio': 1 - theoretical_ql_values,  # ql: 剩余权重比例，1-ql: 丢失权重比例
    'Weight_Remaining_Ratio': theoretical_ql_values,  # ql: 剩余权重比例
...
solutions_file = os.path.join(output_dir, "steady_state_solutions_weight_loss.csv")
solutions_df.to_csv(solutions_file, index=False)
print(f"稳态解已保存至: {solutions_file}")

# 保存每个国家随ql的变化曲线
country_curves_total_df = pd.DataFrame(all_country_steady_states_total, index=simulation_ql_values)
country_curves_total_df.columns = nodes
country_curves_file = os.path.join(output_dir, "country_curves_vs_ql_weight_loss.csv")
country_curves_total_df.to_csv(country_curves_file)
print(f"国家随ql变化曲线已保存至: {country_curves_file}")

# 保存关键点的演化曲线
if ql1_trajectories is not None:
    ql1_trajectories_df = pd.DataFrame(ql1_trajectories, columns=nodes)
    ql1_trajectories_file = os.path.join(output_dir, "country_trajectories_ql1_weight_loss.csv")
    ql1_trajectories_df.to_csv(ql1_trajectories_file, index=False)
    print(f"ql=1.0（原始网络）国家演化曲线已保存至: {ql1_trajectories_file}")

if ql05_trajectories is not None:
    ql05_trajectories_df = pd.DataFrame(ql05_trajectories, columns=nodes)
    ql05_trajectories_file = os.path.join(output_dir, "country_trajectories_ql05_weight_loss.csv")
    ql05_trajectories_df.to_csv(ql05_trajectories_file, index=False)
    print(f"ql=0.5（权重丢失50%）国家演化曲线已保存至: {ql05_trajectories_file}")

if ql01_trajectories is not None:
    ql01_trajectories_df = pd.DataFrame(ql01_trajectories, columns=nodes)
    ql01_trajectories_file = os.path.join(output_dir, "country_trajectories_ql01_weight_loss.csv")
    ql01_trajectories_df.to_csv(ql01_trajectories_file, index=False)
    print(f"ql=0.1（权重丢失90%）国家演化曲线已保存至: {ql01_trajectories_file}")

# ================ 绘图部分 ================
macaron_colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD', '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E9']

# 1. 第一张图：网络总的稳态值<x>随权重丢失比例q_l的变化曲线
print("\n绘制第一张图：网络总的稳态值<x>随权重丢失比例q_l的变化曲线...")

plt.figure(figsize=(14, 10))

# 绘制理论曲线（总贸易量）
theoretical_plot_values_total = [max(val, 1e-5) for val in theoretical_results_total]
plt.plot(1 - theoretical_ql_values, theoretical_plot_values_total,  # 横坐标改为权重丢失比例q_l = 1-ql
         label='Theory-based ODE', color=macaron_colors[1], linewidth=8, linestyle='-')

# 绘制仿真点（总贸易量）
plt.scatter(1 - simulation_ql_values, [max(val, 1e-5) for val in simulation_avg_total_steady],  # 横坐标改为权重丢失比例
...
if len(theoretical_ql_values) > 1 and len(theoretical_results_total) > 1:
    ...
    if zero_indices:
        ...
        for i in range(1, len(theoretical_results_total)):
            if theoretical_results_total[i-1] > threshold_tol and abs(theoretical_results_total[i]) < threshold_tol:
                threshold_q_total = 1 - theoretical_ql_values[i]  # 转换为权重丢失比例
...
if threshold_q_total is not None:
    ...
    # 添加文本标注
    annotation_text = f'$q_l^{{*}}={threshold_q_total:.3f}$'
...
plt.xlabel(r'$q_l$', fontsize=font_size-3)
...
# 保存图片
figure_1_file = os.path.join(figure_dir, "01_total_steady_vs_ql.png")
plt.savefig(figure_1_file, dpi=600, bbox_inches='tight')
print(f"第一张图已保存至: {figure_1_file}")
plt.show()

# 2. 第二张图：进口和出口稳态随q_l的变化曲线
print("\n绘制第二张图：进口和出口稳态随q_l的变化曲线...")

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 8))

# 左侧子图：进口稳态<x_in>随q_l的变化
theoretical_plot_values_in = [max(val, 1e-5) for val in steady_state_solutions_in]
ax1.plot(1 - theoretical_ql_values, theoretical_plot_values_in,  # 横坐标改为权重丢失比例
         label='Theory-based ODE', color=macaron_colors[1], linewidth=5, linestyle='-')

ax1.scatter(1 - simulation_ql_values, [max(val, 1e-5) for val in simulation_avg_in_steady],  # 横坐标改为权重丢失比例
            label='Simulation-based HDE', color=macaron_colors[0],
...
if len(theoretical_ql_values) > 1 and len(steady_state_solutions_in) > 1:
    threshold_tol = 1e-3
    ...
    if zero_indices:
        for i in range(1, len(steady_state_solutions_in)):
            if steady_state_solutions_in[i-1] > threshold_tol and abs(steady_state_solutions_in[i]) < threshold_tol:
                threshold_q_in = 1 - theoretical_ql_values[i]  # 转换为权重丢失比例
...
if threshold_q_in is not None:
    ...
    annotation_text = f'$q_{{l,in}}^{{*}}={threshold_q_in:.3f}$'
...
ax1.set_xlabel(r'$q_l$', fontsize=font_size-3)
...
# 右侧子图：出口稳态<x_out>随q_l的变化
theoretical_plot_values_out = [max(val, 1e-5) for val in steady_state_solutions_out]
ax2.plot(1 - theoretical_ql_values, theoretical_plot_values_out,  # 横坐标改为权重丢失比例
         label='Theory-based ODE', color=macaron_colors[1], linewidth=5, linestyle='-')

ax2.scatter(1 - simulation_ql_values, [max(val, 1e-5) for val in simulation_avg_out_steady],  # 横坐标改为权重丢失比例
            label='Simulation-based HDE', color=macaron_colors[0],
            s=120, marker='^', alpha=0.8, zorder=5)
...
if len(theoretical_ql_values) > 1 and len(steady_state_solutions_out) > 1:
    threshold_tol = 1e-3
    ...
    if zero_indices:
        for i in range(1, len(steady_state_solutions_out)):
            if steady_state_solutions_out[i-1] > threshold_tol and abs(steady_state_solutions_out[i]) < threshold_tol:
                threshold_q_out = 1 - theoretical_ql_values[i]  # 转换为权重丢失比例
...
if threshold_q_out is not None:
    ...
    annotation_text = f'$q_{{l,out}}^{{*}}={threshold_q_out:.3f}$'
...
ax2.set_xlabel(r'$q_l$', fontsize=font_size-3)
...
# 保存图片
figure_2_file = os.path.join(figure_dir, "02_import_export_steady_vs_ql.png")
...
# 5. 第五张图：ql=1.0（原始网络）的演化曲线
print("\n绘制第五张图：ql=1.0（原始网络）的演化曲线...")

if ql1_trajectories is not None:
    plt.figure(figsize=(14, 10))
    
    # 时间轴
    time_axis = np.linspace(0, 100, ql1_trajectories.shape[0])
    
    # 绘制每个国家的演化曲线（浅色细线）
    for i, node in enumerate(nodes):
        if not np.all(np.isnan(ql1_trajectories[:, i])):
            trajectory = ql1_trajectories[:, i]
            # 避免0值
    ...
    # 计算并绘制平均演化曲线
    avg_trajectory = np.nanmean(ql1_trajectories, axis=1)
    # 避免0值
    ...
    # 保存图片
    figure_5_file = os.path.join(figure_dir, "05_trajectories_ql1.png")
    plt.savefig(figure_5_file, dpi=300, bbox_inches='tight')
    print(f"第五张图已保存至: {figure_5_file}")
    plt.show()

# 6. 第六张图：ql=0.5（权重丢失50%）的演化曲线
print("\n绘制第六张图：ql=0.5（权重丢失50%）的演化曲线...")

if ql05_trajectories is not None:
    plt.figure(figsize=(14, 10))
    
    # 时间轴
    time_axis = np.linspace(0, 100, ql05_trajectories.shape[0])
    
    # 绘制每个国家的演化曲线（浅色细线）
    for i, node in enumerate(nodes):
        if not np.all(np.isnan(ql05_trajectories[:, i])):
            trajectory = ql05_trajectories[:, i]
            # 避免0值
            trajectory_plot = np.where(trajectory <= 0, 1e-5, trajectory)
            plt.plot(time_axis, trajectory_plot, 
                    color=macaron_colors[5], alpha=0.6, linewidth=1)
    ...
    # 计算并绘制平均演化曲线
    avg_trajectory = np.nanmean(ql05_trajectories, axis=1)
    # 避免0值
    avg_trajectory_plot = np.where(avg_trajectory <= 0, 1e-5, avg_trajectory)
    plt.plot(time_axis, avg_trajectory_plot, 
    ...
    # 保存图片
    figure_6_file = os.path.join(figure_dir, "06_trajectories_ql05.png")
    plt.savefig(figure_6_file, dpi=300, bbox_inches='tight')
    print(f"第六张图已保存至: {figure_6_file}")
    plt.show()

# 7. 第七张图：ql=0.1（权重丢失90%）的演化曲线
print("\n绘制第七张图：ql=0.1（权重丢失90%）的演化曲线...")

if ql01_trajectories is not None:
    plt.figure(figsize=(14, 10))
    
    # 时间轴
    time_axis = np.linspace(0, 100, ql01_trajectories.shape[0])
    
    # 绘制每个国家的演化曲线（浅色细线）
    for i, node in enumerate(nodes):
        if not np.all(np.isnan(ql01_trajectories[:, i])):
            trajectory = ql01_trajectories[:, i]
            # 避免0值
            trajectory_plot = np.where(trajectory <= 0, 1e-5, trajectory)
            plt.plot(time_axis, trajectory_plot, 
                    color=macaron_colors[5], alpha=0.6, linewidth=1)
    ...
    # 计算并绘制平均演化曲线
    avg_trajectory = np.nanmean(ql01_trajectories, axis=1)
    # 避免0值
    avg_trajectory_plot = np.where(avg_trajectory <= 0, 1e-5, avg_trajectory)
    plt.plot(time_axis, avg_trajectory_plot, 
             label='Average', color=macaron_colors[4], linewidth=3)
    ...
    # 保存图片
    figure_7_file = os.path.join(figure_dir, "07_trajectories_ql01.png")
    plt.savefig(figure_7_file, dpi=300, bbox_inches='tight')
    print(f"第七张图已保存至: {figure_7_file}")
    plt.show()

# 15. 输出改进的韧性动力学方程
print("\n=== 改进的韧性动力学方程（进口和出口分离，使用B_i，分母改为x_mean，权重丢失）===")
print("原始方程:")
...
info_file = os.path.join(output_dir, "network_information_weight_loss.csv")
