...
output_dir = r"E:\论文写作\35 TRE\代码\2 去边\21 按照权重从大到小\数据"
figure_dir = r"E:\论文写作\35 TRE\代码\2 去边\21 按照权重从大到小\作图"
...
# ================ 修改部分1：改为按权重从大到小的去边顺序 ================
# 获取所有边及其权重
edges = list(G_lcc.edges(data=True))

# 按照权重从大到小排序
weight_sorted_edges = sorted(edges, key=lambda x: x[2]['weight'], reverse=True)
...
print(f"最大权重: {weight_sorted_edges[0][2]['weight']:.4f}")
print(f"最小权重: {weight_sorted_edges[-1][2]['weight']:.4f}")

# 保存按权重从大到小的去边顺序
edge_removal_order_df = pd.DataFrame({
    'Removal_Order': range(1, len(weight_sorted_edges) + 1),
    'Source': [edge[0] for edge in weight_sorted_edges],
    'Target': [edge[1] for edge in weight_sorted_edges],
    'Weight': [edge[2]['weight'] for edge in weight_sorted_edges]
})
removal_order_file = os.path.join(output_dir, "weight_sorted_edge_removal_order.csv")
edge_removal_order_df.to_csv(removal_order_file, index=False)
print(f"按权重从大到小的去边顺序已保存至: {removal_order_file}")
...
def theoretical_robustness_analysis_edge_removal(G_lcc, nodes, node_in_volumes, node_out_volumes, B, x_mean, removal_order_edges):
    """改进的理论解析韧性分析 - 使用进口和出口分离的动力学，分母改为x_mean，按权重从大到小去边"""
    ...
    for step, (u, v, attr) in enumerate(removal_order_edges):
        ...
        if (step + 1) % 30 == 0 or step < 10:  # 增加前10步的输出
...
# 7. 改进的韧性分析 - 基于动力学方程的仿真方法（分开进口和出口，使用B_i，分母改为x_mean，按权重从大到小去边）
def simulate_dynamics_robustness_edge_removal(G_lcc, nodes, node_in_volumes, node_out_volumes, E, G_matrix, B_i_values, x_mean, removal_order_edges):
    """基于改进动力学方程的仿真韧性分析 - 分开进口和出口，使用B_i，分母改为x_mean，按权重从大到小去边"""
    n = len(nodes)
...
theoretical_q_values, theoretical_results_total, steady_state_solutions_total, steady_state_solutions_in, steady_state_solutions_out, alpha_values, beta_values = theoretical_robustness_analysis_edge_removal(
    G_lcc, nodes, node_in_volumes, node_out_volumes, B_solution, x_mean, weight_sorted_edges)

print("开始基于改进动力学方程的仿真韧性分析（按权重从大到小去边）...")
simulation_q_values, simulation_avg_total_steady, simulation_avg_in_steady, simulation_avg_out_steady, all_country_steady_states_total, all_country_steady_states_in, all_country_steady_states_out, all_country_trajectories_total, step1_trajectories, step10_trajectories, simulation_alpha_values, simulation_beta_values = simulate_dynamics_robustness_edge_removal(
    G_lcc, nodes, node_in_volumes, node_out_volumes, E, G_matrix, B_i_values, x_mean, weight_sorted_edges)
...
results_file = os.path.join(output_dir, "resilience_analysis_results_edge_removal_weight_sorted.csv")
...
solutions_file = os.path.join(output_dir, "steady_state_solutions_edge_removal_weight_sorted.csv")
...
country_curves_file = os.path.join(output_dir, "country_curves_vs_q_edge_removal_weight_sorted.csv")
...
if step1_trajectories is not None:
    ...
    step1_trajectories_file = os.path.join(output_dir, "country_trajectories_step1_edge_removal_weight_sorted.csv")
...
if step10_trajectories is not None:
    ...
    step10_trajectories_file = os.path.join(output_dir, "country_trajectories_step10_edge_removal_weight_sorted.csv")
